...
int add(int n)
{
    ...
    for(unsigned int currentNumber = 0; currentNumber <= n; currentNumber++){
...
double pie(long int n){
    ...
    //There has to be positive terms or else the estimate is 0 
    if(n <= 0){
    ...
    //current calculated  pie estimate
    //current denominator of current term
    //current term value
    double pieEstimate = 0;
    double denominator = 1;
    double currentTermValue = 1 / denominator;

    for(unsigned int currentTerm = 1; currentTerm <= n; currentTerm++){

        //if the current term is even, subtract
        //else add
        if((currentTerm % 2) == 0){

            pieEstimate -= currentTermValue;
        ...
        //update variables
        denominator += 2;
        currentTermValue = 1 / denominator;

    }

    //multiply the pie estimate by four = 4(1 - 1/3 + 1/5 - 1/7...)
    return pieEstimate * 4;
} 
